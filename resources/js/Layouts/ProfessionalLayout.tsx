...
export default function ProfessionalLayout({ role, children }: { role: 'instructor' | 'coach' | 'parent'; children: ReactNode }) {
    ...
    const parentSelector = isParent && <div className="mb-5 rounded-2xl border border-brand-100 bg-brand-50/70 p-3"><div className="mb-2 text-[0.68rem] font-black text-brand-700">فرزند انتخاب‌شده</div><select className="w-full rounded-xl border border-brand-200 bg-white px-3 py-2 text-xs font-bold text-navy" defaultValue="" onChange={(event) => { if (event.target.value) router.post(`/panel/parent/children/${event.target.value}/select`, {}, { preserveScroll: true }); }}><option value="">انتخاب فرزند</option></select><Link href="/panel/parent#children" className="mt-2 block text-[0.68rem] font-bold text-brand-700">مدیریت فرزندان</Link></div>;
