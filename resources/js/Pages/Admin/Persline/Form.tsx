...
const defaultSettings: Settings = { registration_after: 0, display_mode: 'paged', show_progress: true, randomize_questions: false, allow_multiple_responses: false, allow_back_navigation: true, completion_redirect: '', summary_intro: '', summary_outro: '' };
...
export default function PerslineForm() {
    ...
    return <form onSubmit={submit} encType="multipart/form-data" className="flex flex-col gap-6">
        ...
        <section className="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-navy/5"><div className="mb-5 flex items-center gap-2 text-sm font-black text-navy"><Info className="size-4 text-brand-600" /> اطلاعات و متن‌های فرم</div><div className="grid gap-5 sm:grid-cols-2">
            ...
            <div><label className="mb-1.5 block text-xs font-black text-navy/70">تعداد سؤال قبل از ثبت‌نام با کد پیامکی</label><input type="number" min={0} max={1000} value={form.data.settings.registration_after} onChange={(e) => setSetting('registration_after', Number(e.target.value))} className={input} /><p className="mt-1.5 text-[0.68rem] leading-5 text-navy/45">صفر یعنی اول نام و موبایل + کد پیامک؛ مثلاً ۳ یعنی سه سؤال اول آزاد و بعد ثبت‌نام واقعی با OTP.</p>{form.errors['settings.registration_after'] && <p className="mt-1 text-xs font-bold text-red-600">{form.errors['settings.registration_after']}</p>}</div>
            <div><label className="mb-1.5 block text-xs font-black text-navy/70">نحوه نمایش سؤال‌ها</label><div className="rounded-xl border border-brand-100 bg-brand-50 px-4 py-3 text-sm font-bold leading-7 text-brand-900">هر سؤال در یک صفحه است. تا سؤال فعلی پاسخ نشود، دکمه «سؤال بعدی» فعال نمی‌شود و نمی‌توان سؤال جلوتر را باز کرد.</div></div>
