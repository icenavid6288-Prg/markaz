...
import { ArrowLeft, CheckCircle2, ClipboardList, Send, Sparkles } from 'lucide-react';
...
export default function SurveyShow() {
    ...
    // Build answers record from initial data
    ...
    const progress = Math.min(100, Math.round((answeredCount / Math.max(1, questions.length)) * 100));
    ...
    return <div dir="rtl" className="min-h-screen bg-soft-gray px-4 py-8 md:px-6"><Head title={survey.title} /><main className="mx-auto flex w-full max-w-3xl flex-col gap-5">
        ...
        : questions.length > 0 ? <form onSubmit={submit} className="flex flex-col gap-5">
            {survey.show_progress && <section className="rounded-2xl bg-white p-5 shadow-soft ring-1 ring-navy/5">
                <div className="flex items-center justify-between text-xs font-black text-navy/50"><span>{registrationRequired ? 'مرحله اول از دو مرحله' : 'پیشرفت پاسخ‌گویی'}</span><span>{answeredCount} از {questions.length} پاسخ‌شده</span></div>
            ...
            {questions.map((question, index) => {
                ...
                return <section key={question.id} className="rounded-2xl bg-white p-5 shadow-soft ring-1 ring-navy/5 md:p-6">
                    <div className="flex gap-3"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-brand-100 text-xs font-black text-brand-700">{index + 1}</span><div className="flex-1">
            ...
            <button type="submit" disabled={form.processing} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-l from-brand-600 to-brand-500 px-7 py-4 text-base font-black text-white shadow-glow transition-all hover:from-brand-700 hover:to-brand-600 disabled:cursor-not-allowed disabled:opacity-50">
                ...
                {form.processing ? 'در حال ذخیره...' : 'ثبت و پایان نظرسنجی'}
            </button>
